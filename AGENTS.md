...
internal/equivalence/*         Equivalence-test harness (Phase 8): golden render from sapcc/helm-charts git@pinned-SHA (clone + helm dependency build + helm template) vs. operator-side capture (source.From→Render→transform.Build→Apply); scoped per-resource comparison with per-fixture known-divergences, ignoreLabels, canonicalNamespace; fixtures in testdata/fixtures/<operator>/; four Helm operators gated as default subtests (metal-operator, boot-operator, argora-operator, khalkeon); ipam-capi deferred (needs KustomizeSource path extension)
