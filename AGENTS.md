# dual-deployment-operator - AI Agent Guide
...
internal/source/*              Production Helm ChartLoader (OCI + HTTP(S) repos) and go-git kustomize RootResolver; two-render per reconcile; per-source authSecretRef credentials; opt-in in-memory rendered-manifest cache (Deps.RenderCache, constructed via source.NewRenderCache()) keyed by resolved content id (git SHA / OCI digest / HTTP index digest-or-version); nil cache preserves uncached behavior; fetchSHA uses full-depth fetch (not shallow) so arbitrary historical SHAs resolve. NOTE: the Helm loader does NOT YET run helm-dependency resolution — subcharts must currently be vendored in the pulled .tgz. Adding pull-time resolution (chartutil.Expand + downloader.Manager.Build(), Build-only so Chart.lock is required, single-registry auth) is an accepted, scheduled change: Phase 7.6 (docs/implementation.md), full design in docs/subchart-dependency-resolution.md + context.md Revision 8. Kustomize side is unaffected (krusty resolves remote bases itself).
internal/equivalence/*         Equivalence-test harness (Phase 8): golden render from sapcc/helm-charts git@pinned-SHA (clone + helm dependency build + helm template) vs. operator-side capture (source.From→Render→transform.Build→Apply); scoped per-resource comparison with per-fixture known-divergences, ignoreLabels, canonicalNamespace; fixtures in testdata/fixtures/<operator>/; four Helm operators as RUN_EQUIVALENCE-gated subtests (metal-operator, boot-operator, argora-operator, khalkeon) — opt-in because the golden render needs the internal keppel OCI registry, unreachable from public CI; ipam-capi deferred (needs KustomizeSource path extension)
