...
# Spec: Equivalence Golden Render

## Purpose

Defines how the equivalence harness in `internal/equivalence` produces the "golden" manifest stream from today's `<operator>-remote` wrapper chart in `sapcc/helm-charts`, including cloning at a pinned SHA, rendering, classifying, unwrapping wrapper objects, and excluding delivery plumbing that the operator does not emit.
