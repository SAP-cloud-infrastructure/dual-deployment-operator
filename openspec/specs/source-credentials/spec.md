...
Defines how the operator resolves per-source authentication credentials from a Kubernetes Secret referenced by `authSecretRef`, and how those credentials are applied to OCI Helm pulls, classic HTTP(S) Helm pulls, and git HTTPS clones. Credential material must never appear in logs or error messages.
