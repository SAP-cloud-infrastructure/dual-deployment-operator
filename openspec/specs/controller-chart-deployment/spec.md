...
# Spec: controller-chart-deployment

## Purpose

Defines requirements for chart 1's Deployment resource and associated pod-template configuration: per-shoot namespace scoping, Gardener egress labels, leader election, liveness/readiness probes, metrics wiring, and the writable source-scratch volume needed under a read-only root filesystem.
