...
# Spec: Kustomize Root Resolver

## Purpose

Defines the production `RootResolver` implementation in `internal/source` that fetches a remote git repository into a local temporary directory using `go-git` (pure Go, no `git` binary). The resolver satisfies the existing `RootResolver.Resolve(ctx, url, subPath) (fsPath string, cleanup func(), err error)` seam without changing the interface or the renderer logic.
