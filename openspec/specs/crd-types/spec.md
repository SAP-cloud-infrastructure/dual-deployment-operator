...
`DualDeploymentOperatorSpec` MUST declare exactly four top-level fields with the following Go types and JSON tags:
...
No other top-level fields SHALL be present in `DualDeploymentOperatorSpec` in this change.

#### Scenario: Required fields reject empty spec

- **WHEN** a CR is applied with `spec: {}`
- **THEN** the API server rejects the request
- **AND** the rejection message identifies `spec.source` and `spec.remoteKubeconfig` as required

#### Scenario: Optional fields default to empty

- **WHEN** a CR is applied with only `spec.source` and `spec.remoteKubeconfig` set
