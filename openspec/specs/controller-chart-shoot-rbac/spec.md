...
# Spec: controller-chart-shoot-rbac

## Purpose

Defines requirements for chart 1's shoot-side bootstrap: the gated `shootRbac.enabled` toggle that provisions a Gardener `ManagedResource` (plus backing Secret) delivering the shoot ServiceAccount, applier ClusterRole, and ClusterRoleBinding to the shoot via gardener-resource-manager, plus the token-requestor Secret that Gardener fills with the operator's shoot credentials.
