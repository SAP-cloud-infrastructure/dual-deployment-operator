...
# Spec: Resource Delivery

## Purpose

Defines the `internal/deliver` package: the `Applier` interface, the `SSAApplier` concrete implementation, Server-Side Apply with forced ownership, CR-identity ownership label stamping, cluster-scoped conflict guarding, `caBundle` stripping, GET-after-apply health computation, idempotent delete, and fixed intra-render kind ordering.
