...
# Spec: controller-chart-rbac

## Purpose

Defines requirements for chart 1's seed-side RBAC: the broad applier ClusterRole and ClusterRoleBinding that give the operator's ServiceAccount the permissions it needs to watch CRs, read shoot-access Secrets, apply seed-render kinds (namespaced and cluster-scoped), and hold leader-election Leases.
