...
> **SCOPE REVISION (during apply):** Subchart resolution is restricted to **OCI** dependency repositories. Implementation revealed that `downloader.Manager.Build()` resolves OCI (and `file://`) deps directly but requires classic **HTTP(S)** dependency repos to be pre-registered in `repositories.yaml` + index-cached (`hasAllRepos`/`findChartURL`), which the operator does not do at reconcile time. HTTP(S)-repo subchart deps are now **rejected fail-closed** (`requireLockIfDeps` became `requireResolvableDeps`; commit `deb9b13`), and spec/design updated (commit `6b3d67c`). Consequently **Task 5's online test was removed** — it resolved an HTTP-repo subchart (now rejected) and only passed via a temp-Helm-home workaround for the very limitation this revision drops; OCI resolution stays covered by the hermetic `TestHelmLoaderResolvesUnvendoredDependency` (Task 3). All fleet charts use the keppel OCI registry, so OCI-only covers the real case.
