...
- **Shoot-RBAC bootstrap + token-requestor Secret**: default render emits 0 `ManagedResource` and 0 `token-requestor` Secrets; `--set shootRbac.enabled=true` alone succeeds and defaults the applier identity to the dedicated operator-owned SA `dual-deployment-operator-shoot-applier` (the `required` guard fires only when `serviceAccountName`/`serviceAccountNamespace` are explicitly set empty); the enabled render emits the `dual-deployment-operator-shoot-rbac-bootstrap` `ManagedResource` (+ backing Secret) whose objects are the `dual-deployment-operator-shoot-applier`/`kube-system` `ServiceAccount` + the broad apply-scoped `dual-deployment-operator-shoot-applier` `ClusterRole`+binding (SA created by the bootstrap itself, design.md:942 — the token-requestor Secret mints a token for it and the binding binds to it, so all three ship as one atomic unit), and the `dual-deployment-operator-shoot-access` token-requestor `Secret` (labels `resources.gardener.cloud/purpose: token-requestor`, `class: shoot`; empty `token`/`bundle.crt`); `--set shootRbac.secretName=<custom>` renames the Secret. The applier SA is deliberately dedicated (NOT the workload's `metal-operator-controller-manager`) to avoid the CR-delete self-deauthentication finalizer deadlock; the workload keeps its own token-requestor Secret `metal-operator-remote-kubeconfig` in `-v2` (commit `871dd0c`). Templates are hand-authored under `chart/templates/shoot-rbac/` (not plugin-generated), recorded in `chart/README.md` for `--force` re-apply.
