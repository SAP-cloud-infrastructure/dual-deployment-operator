...
**Task 7 addendum (2026-07): real authed-success tests + CI workflow**

The original Task 7 only proved credential _mapping_ (the authed git test returned a 403; the authed OCI test skipped without env). The spec required "authenticated … succeeds" — a gap. This addendum closes it without changing production code.

- [x] **Addendum A: Real authed git clone succeeds (offline/hermetic)**

  Added `TestGitResolverAuthedCloneSucceeds` to `internal/source/gitresolver_authed_test.go`.
  Approach: `net/http/cgi` wraps `git-http-backend` (located via `git --exec-path`) in an
  `httptest.Server` enforcing Basic-auth. A real bare repo is cloned to it. The test asserts:
  - Correct creds → `Resolve` returns no error; `seed/kustomization.yaml` exists.
  - Wrong creds → error (401).
  No build tag; test is offline/hermetic (localhost only). Skips when `git-http-backend` is absent.

- [x] **Addendum B: Authed OCI pull via registry:2 in CI**

  The production `pullOCI` lacks `registry.ClientOptPlainHTTP()`, so it cannot pull from a
  plain-HTTP `localhost:5000` registry (ORAS GHSA-vh4v-2xq2-g5cg prevents credential forwarding
  across an implicit HTTP downgrade). Workaround: CI workflow runs registry:2 with a self-signed
  TLS cert; the runner trusts it system-wide via `update-ca-certificates`. No production code
  change required; the limitation is documented.

  `TestHelmLoaderOCIAuthed` (existing, `//go:build online`) works against this registry with
  `DDO_TEST_OCI_URL=oci://localhost:5443/charts` + the fixture chart pushed by the workflow.

- [x] **Addendum C: Hand-authored `.github/workflows/online-tests.yaml`**

  Created `online-tests.yaml` (NOT autogenerated by go-makefile-maker). Runs on pull_request,
  push to main, workflow_dispatch. Single job: generates self-signed cert + htpasswd, starts
  registry:2 (HTTPS), pushes a fixture chart, runs `make test-online` with the full
  `DDO_TEST_OCI_*` env. All tests that do not require external network pass: anonymous OCI/HTTP,
  anonymous kustomize git, authed git (hermetic), authed OCI (registry:2).
