...
- **Helm OCI pull, anonymous (primary)** — `Load` a real small chart from `oci://ghcr.io/stefanprodan/charts` at a pinned version with **no credentials** and assert the returned `*chart.Chart` parses. Runs under `DDO_ONLINE_TESTS=1`.
- **Helm OCI pull, authenticated** — `TestHelmLoaderOCIAuthed` (`//go:build online`, gated on `DDO_TEST_OCI_*`). The CI job `online-tests.yaml` (hand-authored, NOT generated by go-makefile-maker) starts registry:2 with TLS on `localhost:5443` (self-signed cert trusted system-wide via `update-ca-certificates`), generates htpasswd, pushes a fixture chart, and runs the test with `DDO_TEST_OCI_URL=oci://localhost:5443/charts`. Note: `pullOCI` does not add `registry.ClientOptPlainHTTP()`, so plain-HTTP `localhost:5000` fails auth (ORAS GHSA-vh4v-2xq2-g5cg credential-forwarding protection); TLS is the CI workaround without touching production code. Adding `ClientOptPlainHTTP()` is deferred to a future change.
- **Auth failure** — wrong credentials return a clear, non-leaking error; asserted by both `TestGitResolverHTTPSBasicAuth` (offline) and `TestHelmLoaderOCIAuthed` (online, wrong-password variant).
- **Authenticated git clone succeeds (offline/hermetic)** — `TestGitResolverAuthedCloneSucceeds` (`internal/source/gitresolver_authed_test.go`, NOT build-tagged). Stands up an in-process `git-http-backend` CGI handler (via `net/http/cgi`) guarded by Basic-auth inside an `httptest.Server`. Proves correct creds → resolve succeeds + `seed/kustomization.yaml` exists; wrong creds → error (401). Skips gracefully when `git-http-backend` is absent.
- **kustomize git resolve, anonymous** — `TestKustomizeOnlineRender` fetches a pinned-ref overlay from `github.com/kubernetes-sigs/kustomize` (public) and asserts the overlay renders. Runs under `DDO_ONLINE_TESTS=1`.
- **Offline still green** — `make test` without `DDO_ONLINE_TESTS` skips the network tests; offline tests (local bare repo, local fixtures) pass unchanged.

**CI shape**: `ci.yaml` / `checks.yaml` / `codeql.yaml` are autogenerated by go-makefile-maker (do NOT hand-edit). Online tests run in the hand-authored `.github/workflows/online-tests.yaml` which coexists with the autogenerated workflows.

**Success criterion**: the operator pulls a real (small) chart **anonymously** from a public OCI registry and fetches a real pinned-ref kustomize overlay from the public `github.com/kubernetes-sigs/kustomize` source; the authenticated credential seam is proven by a hermetic in-process git server (offline) and by a CI registry:2 job (OCI authed pull); `make test` stays green offline while the online tier passes in CI.
