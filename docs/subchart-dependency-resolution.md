...
**Related design:** see `design.md` §3 (source discriminator, Helm loader) and `context.md` (this doc adds "Revision 8").
...
This proposal adds an opt-in dependency-build step to the loader so charts can declare `dependencies:` in `Chart.yaml` and have the operator resolve them at pull time — removing the "publish a fat `.tgz` with `charts/*.tgz` vendored" requirement. It is **~35–40 lines of Go in `internal/source/helmloader.go`**, no CRD change, no new transformation.

This is a **proposal only**; no code lands with this document. The first consumer (`metal-operator-remote-v2`) ships with its subchart **vendored** so it works against the operator as it exists today; adopting this enhancement later lets that chart (and future ones) drop vendoring.
...
Gating: this can be unconditional (a chart with no `dependencies:` and no `Chart.lock` is a cheap no-op through `Manager.Build()`), or gated on the presence of `Chart.yaml` `dependencies`. Prefer unconditional for simplicity unless the reconcile-time network cost (below) argues otherwise.

---

## 4. Risks

1. **Single-registry auth.** The CR carries one `authSecretRef` for pulling the chart. If a subchart lives in a **different** private registry, `Manager.Build()` cannot authenticate to it — there is no per-dependency credential. Charts must keep subcharts in the same (or a public) registry, or continue vendoring.
2. **Network at reconcile time.** Resolving dependencies requires egress from the operator Pod to the subchart repo on every uncached render. This needs NetworkPolicy allowance and adds a failure mode (subchart repo unreachable → render fails) that vendoring does not have.
3. **`Chart.lock` semantics.** With a committed `Chart.lock`, `Manager.Build()` resolves the pinned versions deterministically. Without it, `Build()` degrades toward `Update()`-style semver resolution against the repo index — non-deterministic and more network-dependent. Charts using this path should commit `Chart.lock`.

The render cache (keyed on the resolved content id) mitigates (2)/(3) for steady state, but the first render per content id still pays the cost.

---

## 5. Decision framing

- This document is a **proposal**; no code is included with it. It is tracked here so a session working in this repo can pick it up as a self-contained `internal/source` change.
- It is **not required** by `metal-operator-remote-v2`: that chart vendors its subchart and works against the operator as it exists today. This enhancement is the simplification that lets `-v2` (and future wrapper charts) drop vendoring once merged.
- Recommended sequencing (per the helm-charts side): treat this as a **separate, prerequisite** operator change done first; the `-v2` chart then chooses vendored-`.tgz` (safe default) or a plain `dependencies:` entry depending on whether this has shipped.
