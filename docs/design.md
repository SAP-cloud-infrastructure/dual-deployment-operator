...
**Subchart dependency resolution (Phase 7.6).** A wrapper chart may declare the upstream operator's chart as a Helm **subchart** `dependency` in `Chart.yaml` (as `metal-operator-remote-v2` does) rather than vendoring it. The loader resolves such dependencies at pull time: it expands the pulled `.tgz` and runs Helm's `downloader.Manager.Build()` on the chart directory before loading, so a plain `dependencies:` entry renders correctly without a pre-vendored `charts/` directory. Two constraints follow, both documented author contracts rather than operator features:

- **`Chart.lock` is required.** The loader calls `Build()` only (never `Update()`); Helm's `Build()` silently falls back to semver re-negotiation against the live index when no lock is present, which would make renders non-deterministic. A committed `Chart.lock` pins exact subchart versions; a missing/out-of-sync lock fails the render closed (surfaced on CR status) rather than under-rendering.
- **Single-registry auth.** The CR's one `authSecretRef` authenticates the parent pull only; Helm supports one credential set per OCI host and no per-dependency credentials ([helm/helm#11286](https://github.com/helm/helm/issues/11286)). Subcharts must live on the same (or a public) registry as the parent, or stay vendored. This is the community-standard reconcile-time resolution model (Argo CD resolves dependencies at sync time identically). See §3.3's kustomize note below for why the kustomize path needs no equivalent step, and [`subchart-dependency-resolution.md`](subchart-dependency-resolution.md) for the full design.
...
**No dependency-resolution step needed (contrast with Helm §3.3 above).** The kustomize path has no equivalent of the Helm silent-under-render gap: `krusty.Run` (invoked with `LoadRestrictionsNone`) fetches remote bases itself during the build — when a `kustomization.yaml` references a base by URL (e.g. ipam-capi's `github.com/kubernetes-sigs/cluster-api-ipam-provider-in-cluster//config/*?ref=v1.1.0`), krusty resolves it as part of rendering. The git `RootResolver` supplies only the pinned root and is fail-closed on an unresolvable `?ref=`. Helm's `loader.Load()`, by contrast, is a pure unpack that never fetches declared-but-absent `dependencies:`, which is why only the Helm loader needs the Phase 7.6 `downloader.Manager.Build()` step. The one kustomize nuance — a force-moved upstream tag changing what renders under an unchanged root SHA — is a cache-soundness caveat handled in the source cache (transitive-tag mutability), not a rendering gap.
...
1. **`dual-deployment-operator`** (the *controller* / **upstream chart**, lives in **this repo** at `chart/`): the operator Deployment, its ServiceAccount, seed-side RBAC, leader-election Role, NetworkPolicy, and the `DualDeploymentOperator` **CRD definition** shipped with the chart. The kubebuilder `helm/v2-alpha` plugin emits the CRD as a **toggled template** at `chart/templates/crd/…` gated by `.Values.crd.enabled` (default `true`), with `.Values.crd.keep` (default `true`) stamping `helm.sh/resource-policy: keep` so the CRD is retained on `helm uninstall` — the plugin's native layout, accepted so the chart stays fully generated (an untemplated `chart/crds/` dir cannot be conditioned or keep-annotated). Generated and maintained via the kubebuilder helm plugin, written to the repo-root `chart/` directory via the plugin's `--output-dir` flag (`kubebuilder edit --plugins=helm/v2-alpha --output-dir=.`), versioned with the operator image, and published as an OCI chart (e.g. `oci://keppel.global.cloud.sap/ccloud-helm/dual-deployment-operator`) alongside the operator image — exactly how the upstream operators publish their charts. This repo owns it; it is the operator's self-contained deployment artifact.
