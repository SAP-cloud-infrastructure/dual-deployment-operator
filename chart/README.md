...
- **Shoot-applier bootstrap** (`templates/shoot-rbac/applier-bootstrap.yaml`, `values.yaml` `shootRbac`): hand-authored, NOT plugin-generated — re-add after any `--force` regen. Gated `shootRbac.enabled: false` by default; when enabled (by the wrapper chart, with `serviceAccountName`/`serviceAccountNamespace` supplied) it emits a GRM `ManagedResource` that seeds the broad apply-scoped shoot ClusterRole+binding for the operator's shoot SA (design.md §3.6.7). The webhook-injector shoot RBAC stays in the workload chart, not here.
